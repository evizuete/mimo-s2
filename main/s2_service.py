...
class S2Service:
    ...
    def _build_current_indicators(
        ...
        put(
            "proba_long",
            # FIX 17/04/2026 BUG-4: pip_last tiene prioridad sobre live_order.
            # live_order.proba_long es la proba_long_cal calculada por el simulador
            # en la última barra en que no había posición abierta — puede llevar
            # 60-90 minutos sin actualizarse (observado: mismo valor 0.2095 durante
            # toda la sesión mientras proba_long_raw variaba barra a barra).
            # pip_last contiene el valor fresco del pipeline de features actual.
            # Si pip_last no está disponible se cae al valor de live_order como
            # fallback, y finalmente a model_diag (proba calibrada del diag).
            pip_get(pip_last, "proba_long") if pip_get else None,
            model_diag.get("proba_long_cal") if isinstance(model_diag, dict) else None,
            live_order.get("proba_long") if live_order else None,
            model_diag.get("proba_long") if isinstance(model_diag, dict) else None,
        )
        put(
            "proba_short",
            pip_get(pip_last, "proba_short") if pip_get else None,
            model_diag.get("proba_short_cal") if isinstance(model_diag, dict) else None,
            live_order.get("proba_short") if live_order else None,
            model_diag.get("proba_short") if isinstance(model_diag, dict) else None,
        )
