...
def main():
    ...
    threshold = 0.4960
    for daily_loss in np.linspace(0.0050, 0.0450, num=8):
        print(f'=' * 160)
        print(f'Max daily loss: {daily_loss:.4f}')
        res_train = evaluate(from_date=date0, to_date=date1, mode='train', max_daily_loss_pct=daily_loss,
                             debug=True, general_config=general_config, model_config=model_config,
                             feature_config=feature_config, regime_config=regime_config, decision_policy=decision_policy,
                             risk_config=risk_config, artifacts_path=artifacts_path, use_rl=True, rl_config=rl_config,
                             rl_train=True, rl_eval_deterministic=False, rl_take_threshold=threshold)

        res_eval_month1 = evaluate(from_date=date1, to_date=date1 + timedelta(days=30), mode='eval',
                                   max_daily_loss_pct=daily_loss, debug=True, general_config=general_config,
        ...
        res_eval_month2 = evaluate(from_date=date1 + timedelta(days=30), to_date=date1 + timedelta(days=60), mode='eval',
                                   max_daily_loss_pct=daily_loss, debug=True, general_config=general_config,
                                   model_config=model_config, feature_config=feature_config,
                                   regime_config=regime_config,
                                   decision_policy=decision_policy, risk_config=risk_config,
                                   artifacts_path=artifacts_path,
                                   use_rl=True, rl_config=rl_config, rl_train=False, rl_eval_deterministic=True,
                                   rl_take_threshold=threshold)

        res_eval_month3 = evaluate(from_date=date1 + timedelta(days=60), to_date=date1 + timedelta(days=90), mode='eval',
                                   max_daily_loss_pct=daily_loss, debug=True, general_config=general_config,
                                   model_config=model_config, feature_config=feature_config,
                                   regime_config=regime_config,
                                   decision_policy=decision_policy, risk_config=risk_config,
