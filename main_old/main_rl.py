...
class RLOptunaOptimizer:
    def __init__(
        ...
        if study_name is None:
            self.study_name = f'rl_study_{self.release}'
        else:
    ...
    def optimize(
        ...
        if early_stopping_rounds:
            ...
            callbacks.append(
                self._create_early_stopping_callback(early_stopping_rounds, min_improvement)
            )
    ...
    def _create_early_stopping_callback(self, patience: int, min_improvement: float) -> Callable:
        def callback(study, trial):
            ...
            if improvement < min_improvement:
                study.stop()
                self.logger.info(f'\n⏹ Early stopping: no improvement > {min_improvement} in {patience} trials\n')
    ...
    def print_study_summary(self, top_n: int = 5):
        print(f'\n{"=" * 80}')
        print(f'OPTIMIZATION REPORT: {self.study_name}')
        print(f'\n{"=" * 80}')
        ...
        print(f'\n{"-" * 80}')
        print('SUMMARY OF METRIC STATISTICS:')
        print(f'\n{"-" * 80}')
...
def build_training_command(params: Dict[str, Any]) -> List[str]:
    """Construye el comando para ejecutar main_rl_train.py"""
...
def parse_training_output(output: str) -> Dict[str, float]:
    """
    Parsea la salida del script para extraer métricas.

    El script main_rl_train.py imprime:
        Trades     : 695
        NetPnL     : 1273.30
        ProfitFactor: 1.0738938378901903
        WinRate    : 0.3294964028776978
        MaxDD      : 1728.9218479123629
        MaxDD (%)  : -0.1483
    """
    results = {}
...
def run_rl_training(params: Dict[str, Any], verbose: bool = True) -> Dict[str, float]:
    """
    Ejecuta el entrenamiento RL y devuelve las métricas.

    MEJORAS:
    1. Imprime stderr además de stdout para debugging
    2. Guarda el output completo en un archivo log
    ...
    try:
        ...
        log_dir = Path('./artifacts/optimization_logs')
        ...
        if not metrics or 'net_pnl' not in metrics:
            print(f'⚠ WARNING: Metrics were not parsed.')
            print(f'Last 1000 chars of output:')
            print(result.stdout[-1000:])
            print(f'\nFull log saved to: {log_file}')

            # Valores por defecto muy malos
            return {
                'net_pnl': -999999.0,
                'profit_factor': 0.0,
                'win_rate': 0.0,
        ...
        # Si falta max_dd_pct, calcularla o usar 0
        if 'max_dd_pct' not in metrics:
        ...
        print(f"❌ ERROR: Training timed out after 3600 seconds")
        return {
            'net_pnl': -999999.0,
            'profit_factor': 0.0,
            'win_rate': 0.0,
...
if __name__ == '__main__':
    ...
    filepath = f'{dir_prefix}_{release}'
    ...
    # Exportar resultados
    optimizer.export_best_config(f'./artifacts/{filepath}/best_rl_config.json')
    optimizer.export_all_trials(f'./artifacts/{filepath}/all_trials.csv')
    ...
    try:
        optimizer.plot_optimization_history().write_html(
            f'./artifacts/{filepath}/optimization_history.html')
        optimizer.plot_param_importances().write_html(
            f'./artifacts/{filepath}/param_importances.html')
        optimizer.plot_parallel_coordinate().write_html(
            f'./artifacts/{filepath}/parallel_coordinate.html')
        optimizer.plot_contour().write_html(f'./artifacts/{filepath}/contour.html')
        print(f"✓ Plots saved on ./artifacts/{filepath}")
    ...
    pareto_df.to_csv(f'./artifacts/{filepath}/pareto_front.csv', index=False)
    print(f"✓ Pareto front saved on ./artifacts/{filepath}/pareto_front.csv")
