...
def trading_engine(release: str, trading_policy: str = 'aggressive'):
    ...
    risk_config = RiskConfig(
        ...
        min_score_to_trade=0.15, #0.2898742140685939
        max_risk_pct=0.02,
        max_positions=2
    )
...
while True:
    ...
    if live_order:
        ...
        #    Si hay una posición abierta en pérdida > 0.5R y la nueva señal es
        #    opuesta, bloquear: probablemente el mercado está en contra y el
        #    modelo está sobreajustando al momentum reciente.
        if n_positions > 0 and floating < -50:
            mt5_side = 'BUY' if side == 'long' else 'SELL'
            # Detectar si la señal actual es opuesta a la posición en pérdida
            # (no tenemos el side de la posición abierta directamente, pero
            #  si floating < -50 y el modelo acaba de cambiar de dirección es señal de alarma)
            prev_signal = getattr(trading_engine, '_last_side', None)
            if prev_signal and prev_signal != side:
                block_reason = f'OPPOSITE_SIDE_IN_DRAWDOWN(floating={floating:.2f})'

        # 2. Score muy bajo con posición ya abierta: no acumular con señales débiles
        if n_positions > 0 and score < 0.20 and block_reason is None:
            block_reason = f'LOW_SCORE_WITH_OPEN_POS(score={score:.4f})'

        if block_reason:
            log_signal(signal_logger, event='SIGNAL_BLOCKED',
                       bar_data=bar_snapshot, live_order=live_order,
                       block_reason=block_reason,
                       n_positions=n_positions, equity=equity, balance=balance)
        ...
        # Guardar el último side enviado (o bloqueado) para detectar flip de dirección
        trading_engine._last_side = side
        ...
        log_signal(signal_logger, event='NO_SIGNAL',
                   bar_data=bar_snapshot, n_positions=n_positions,
