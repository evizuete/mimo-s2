...
from mimo.databases import Database
from mimo.decision_engine_v2 import DecisionPolicy, RiskConfig
from mimo.feature_builder import FeatureConfig
from mimo.model_builder import ModelConfig, Config
from mimo.regime_detector import RegimeConfig
from mimo.trading_simulator_v2 import TradingSimulator, load_rl_policy_npz
...
def create_order_request(
    ...
    request = {
        ...
        # Se extraen de 'order' (live_order de decide_live); son opcionales:
        # si no están presentes el adaptativo ignora esa señal concreta.
...
class S3State:
    ...
    def on_event(self, msg: Dict) -> None:
        ...
        with self._lock:
            ...
            elif event == 'ADAPTIVE_SL_CLOSE':
                # El adaptativo cerró la posición (expansión agotada o compresión final)
                ticket = int(msg.get('ticket', -1))
                self.positions.pop(ticket, None)
                if 'slippage_pts' in msg:
                    self.last_slippage_pts = float(msg['slippage_pts'])
            ...
            # ── Eventos AdaptiveTPManager (v9.0) ─────────────────────────────
                ...
                # El adaptativo cerró por TP extendido o SL post-extensión
                ticket = int(msg.get('ticket', -1))
                self.positions.pop(ticket, None)
...
def s3_event_listener(sub_socket: zmq.Socket, s3_state: S3State) -> None:
    ...
    while True:
        try:
            ...
            # Timeout normal (no hay eventos nuevos de S3) — yield CPU
            time.sleep(0.01)
        except Exception:
            time.sleep(0.1)
...
def trading_engine(release: str, trading_policy: str = 'aggressive'):
    ...
    feature_config = FeatureConfig(
        ...
        label_horizon=5,
        tp_barrier=2.4,
        sl_barrier=1.2, # Para evitar que en aquellos casos en que el Virtual SL es muy justo, cierre a perdidas
...
events_sub.setsockopt(zmq.RCVTIMEO, 5_000)  # v11.0: 5s timeout — evita que el thread background se cuelgue
...
def _db_save_async(df_to_save):
    """Guarda en BD en background. Los errores se loguean pero no crashean el loop."""
    try:
...
while True:
    ...
    _db_executor.submit(_db_save_async, df.copy())  # v11.0: asíncrono — no bloquea el loop
    ...
    # ── Extraer indicadores del último bar para AdaptiveSLManager (v8.0) ────
    # decide_live puede dejar _last_diag con los indicadores del bar actual.
    # Intentamos extraerlos tanto de live_order como del diagnóstico interno.
    _diag = getattr(engine, '_last_diag', None) or {}
    _ind_source = live_order or _diag
    current_indicators = {k: v for k, v in {
        'atr':            data.get('atr') or (_ind_source.get('atr_at_entry')),
        'rsi':            data.get('rsi') or _ind_source.get('rsi'),
        'macd_hist':      data.get('macd_hist') or _ind_source.get('macd_hist'),
        'macd_hist_prev': _ind_source.get('macd_hist_prev'),
        'volume':         data.get('volume'),
        'volume_ma':      _ind_source.get('volume_ma'),
        'proba_long':     _ind_source.get('proba_long') or _ind_source.get('proba_cal'),
        'proba_short':    _ind_source.get('proba_short'),
    }.items() if v is not None}

    # Enviar indicadores frescos a S3 para cada posición abierta (v8.0)
    # El AdaptiveSLManager los usará en su próximo ciclo on_bar().
    if current_indicators:
        for ticket in s3_state.get_positions():
            send_order(orders_push, {
                'action': 'MODIFY',
                'ticket': ticket,
