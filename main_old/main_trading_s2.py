...
def trading_engine(release: str, trading_policy: str = 'aggressive'):
    ...
    rl_config = {
        "lr": 0.00011492999300221417,
        "entropy_coef": 0.00782428325709939,
        "baseline_beta": 0.8670524123687291,
        "max_grad_norm": 3.7336180394137353,
        "trade_cost_money": 0.007608811483056596,
        "batch_size": 128,
        "chop_soft_thr": 0.609757738947792,
        "exhaustion_soft_thr": 0.6448448049611839,
        'rl_take_threshold': 0.25557975442608166,
        "chop_penalty_coef": 0.003086057974053536,
        "exhaustion_penalty_coef": 0.012152721946343493,
    }
    ...
    engine = TradingSimulator(
        ...
        rl_take_threshold=0.3042703315240835, #0.62,
...
while True:
    ...
    if hasattr(engine, 'pipeline') and hasattr(engine.pipeline, 'live_update_scalers_from_df'):
        ...
        engine.pipeline.live_update_scalers_from_df(df_prepared)
    ...
    if live_order:
        ...
        def create_order_request(
            ...
            request = {
                ...
                'risk': {
                    # SL de emergencia - ÚNICO que se envía al broker
                    'emergency_points': emergency_points,

                    # Break-even (basado en tp_points)
                    'be_trigger_points': be_trigger_points,
                    'be_offset_points': be_offset_points,

                    # Trailing stop (basado en tp_points)
                    ...
                    'partial_close': {
                        ...
                        'basis': 'R' # R se calcula con virtual_sl_points
                    }
                }
            }

            request['risk']['close_confirmation'] = {
                'exit_mode': 'soft',
                'tp_mode': 'close',
                'tp_confirm_count': 1,
                'sl_mode': 'touch',
