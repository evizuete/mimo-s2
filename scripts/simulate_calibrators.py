...
Outputs:
  ...
  - Tabla de hit-rate por bucket
  - Tabla campeón por (side, state)
  - JSON con mapping (side, state) → champion_method
...
def main():
    ...
    print(f"\n{'═'*110}\n  CAMPEÓN POR (side, state) — minimizando ECE\n{'═'*110}")
    champion_rows = []
    ...
    if champion_rows:
        ...
        if n_unique_winners == 1 and ch_df["champion"].iloc[0] == args.baseline_method:
            ...
            if (sides_winners == 1).all():
                print(f"     🟢 Cada side prefiere UN método. Nivel 1 heterogéneo por side basta.")
                for side in ch_df["side"].unique():
                    method = ch_df[ch_df["side"] == side]["champion"].iloc[0]
                    print(f"        {side} → {method}")
            else:
                print(f"     🟠 Heterogeneidad dentro de algún side. Considerar Nivel 2.")
        else:
            print(f"     🔴 Heterogeneidad alta ({n_unique_winners} campeones). Nivel 2 con cautela.")
