...
class TradingSimulator:
    ...
    def predict_live(self, df_rates: pd.DataFrame, verbose: bool = False) -> pd.DataFrame:
        """
        Versión optimizada de predict() para inferencia en vivo (producción).

        DIFERENCIA CLAVE respecto a predict():
        ─────────────────────────────────────
        predict() construye secuencias para TODAS las filas del historial y luego
        llama a model.predict() sobre el array completo. Con 2048 filas de historial
        y seq_len_long=256 eso produce ~1793 secuencias (o más, según el pipeline),
        que con batch_size=8192 se traducen en 42 steps × ~61ms = ~7s por tick.

        predict_live() construye el mismo pipeline pero solo pasa al modelo
        LA ÚLTIMA SECUENCIA: un tensor de shape (1, seq_len, n_features).
        Un único forward pass en GPU tarda <100ms independientemente del historial.

        El resultado es un DataFrame de UNA sola fila (la última barra) con las
        mismas columnas que predict(): pred_long_raw, pred_long_cal,
        pred_short_raw, pred_short_cal, state, market_condition, etc.
        decide_live() solo necesita df_pred.iloc[-1], así que es 100% compatible.

        Requisitos:
        - df_rates debe tener al menos seq_len_long + warmup filas (recomendado ≥ 400).
        - No usa caché (siempre computa fresh, igual que predict(simulation=False)).
        - Los scalers del pipeline deben estar ya ajustados (load_artifacts() llamado).

        Args:
            df_rates:  DataFrame con datos OHLCV, al menos 400 filas recomendadas.
            verbose:   Si True, imprime feature ranges del último step (igual que predict).

        Returns:
            DataFrame de 1 fila con predicciones + columnas de features de la barra actual.
        """
        if not self.loaded:
            self.load_artifacts()

        # ── 1. prepare_data sobre el historial completo ──────────────────────
        # Necesario para calcular correctamente EMAs, RSI, MACD, ADX, regime…
        # con el warmup adecuado. El coste es ~50-100ms para 400 filas y es inevitable
        # porque los indicadores técnicos tienen memoria (dependen de barras anteriores).
        df_p = self.pipeline.prepare_data(
            df_rates,
            labels=False,
            side='both',
        ...
        if self.enable_live_scaler_updates:
            # live_update_scalers_from_df espera el df preparado con side=None (16 features).
            # Con side='both' el pipeline añade columnas extra para ambos lados (19 features)
            # que el scaler no conoce → ValueError: n_features mismatch got 19 expected 16.
            # Reproducimos exactamente lo que hace _prepare_live: prepare_data con side=None.
            # El coste es mínimo porque prepare_data es O(n_rows) y ya tenemos el df cacheado
            # en memoria — solo recalcula las columnas de features, no los scalers ni secuencias.
            try:
                df_p_scaler = self.pipeline.prepare_data(
                    df_rates,
                    labels=False,
                    side=None,
                    set_market_condition=True,
                ...
                df_p_scaler.dropna(inplace=True)
                df_p_scaler.reset_index(drop=True, inplace=True)
                if len(df_p_scaler) > 0:
                    self.pipeline.live_update_scalers_from_df(df_p_scaler)
            except Exception as _e:
                print(f'[WARN][predict_live] live_update_scalers_from_df failed: {_e}')

        # ── 2. Construir secuencias solo para el último bar ───────────────────
        # create_sequences_by_side genera arrays de shape (N, seq_len, features).
        # Necesitamos ÚNICAMENTE el último sample [-1:] de cada array.
        # Esto evita pasar N-1 secuencias inútiles al modelo.
        sequences = self.pipeline.create_sequences_by_side(
            df_p,
            sides=('long', 'short'),
            fit_scalers=False,
            train=False,
        ...
        # ── 3. Inferencia con batch de 1 muestra por modelo ─────────────────
        # Sliceamos [-1:] de cada tensor → shape (1, seq_len, features).
        # model.predict() con 1 muestra hace un único forward pass en GPU: <100ms.
        results = {}
        for side in ('long', 'short'):
            pack = sequences[side]
            n = len(pack['seq_long'])
            if n == 0:
                results[side] = (None, None)
                continue

            # Solo la última secuencia
            X_short   = pack['seq_short'][-1:]
            X_long    = pack['seq_long'][-1:]
            X_context = pack['context'][-1:]
            X_time    = pack['time'][-1:]
            ...
            p_cal = np.clip(p_cal, 0.0, 1.0)
            results[side] = (p_raw, p_cal)

        # ── 4. Construir DataFrame de 1 fila alineado con la última barra ────
        # context_offset = seq_len_long - 1: la primera secuencia válida corresponde
        # a la fila df_p.iloc[seq_len_long - 1]. La última secuencia → df_p.iloc[-1].
        L = int(getattr(self.model_config, 'seq_len_long', 64))
        last_row = df_p.iloc[[-1]].copy()   # DataFrame de 1 fila

        p_raw_long,  p_cal_long  = results['long']
        p_raw_short, p_cal_short = results['short']

        if p_raw_long is None or p_raw_short is None:
            raise ValueError('[predict_live] No se pudieron generar predicciones: '
                             f'df_rates tiene muy pocas filas (mínimo {L}).')

        last_row['pred_long_raw']   = float(p_raw_long[0])
        last_row['pred_long_cal']   = float(p_cal_long[0])
        last_row['pred_short_raw']  = float(p_raw_short[0])
        last_row['pred_short_cal']  = float(p_cal_short[0])

        # Exponer df_p para que el caller (S2) pueda reutilizarlo sin recalcular.
        # S2 v20+ lo lee desde engine._last_df_prepared para extraer RSI/MACD/ATR
        # del último bar sin necesidad de llamar pipeline.prepare_data() de nuevo.
        self._last_df_prepared = df_p

        # Normalización de columnas — igual que predict() para compatibilidad total
        if 'time' not in last_row.columns and isinstance(last_row.index, pd.DatetimeIndex):
            last_row['time'] = last_row.index

        if 'macro_regime' not in last_row.columns and 'regime' in last_row.columns:
            last_row['macro_regime'] = last_row['regime'].astype(str)

        if 'state' in last_row.columns:
            last_row['market_condition'] = last_row['state'].astype(str)
        elif 'macro_regime' in last_row.columns:
            last_row['market_condition'] = last_row['macro_regime'].astype(str)
        elif 'regime' in last_row.columns:
            last_row['market_condition'] = last_row['regime'].astype(str)
        else:
            last_row['market_condition'] = ''
    ...
    Optional[Dict[str, Any]]:
        ...
        # Resolución de barriers según side — SHORT puede tener barriers propios
        # para alinear la geometría de ejecución con el método de labeling usado
        # durante el entrenamiento (adaptive para SHORT, triple_barrier para LONG).
        if decision.action == "sell":
            sl_mult = float(
                getattr(self.feature_config, "sl_barrier_short", None)
                or getattr(self.feature_config, "sl_barrier", 1.0)
            )
            tp_mult = float(
                getattr(self.feature_config, "tp_barrier_short", None)
                or getattr(self.feature_config, "tp_barrier", 1.0)
            )
        else:
    ...
    def decide_live(self,
        ...
        # OPTIMIZACIÓN: predict_live() pasa únicamente la última secuencia al modelo
        # (batch_size=1, shape (1, seq_len, features)) en lugar de las ~1793 secuencias
        # que genera predict() sobre 2048 filas de historial.
        # Esto reduce la inferencia de ~7s (42 batches × 61ms) a <100ms por tick.
        # El resultado es un DataFrame de 1 fila con exactamente las mismas columnas
        # que predict() — 100% compatible con decide_at() y print_last() más abajo.
        df_pred = self.predict_live(df_rates, verbose=True)
        ...
        # FIX BUG-4 (17/04/2026): poblar _last_diag también en el path de éxito.
        # Cuando decide_live retorna una orden, _last_diag quedaba a None (solo se
        # poblaba en los paths de rechazo vía _reject()). Esto hacía que s2_service
        # leyera model_diag=None y cayera al fallback live_order.proba_long (stale).
        # Con este bloque, model_diag siempre tiene proba_long_cal fresco del diag.
        self._last_diag = {
            "proba_long_raw":   round(float(getattr(row, "pred_long_raw",  0) or 0), 6),
            "proba_short_raw":  round(float(getattr(row, "pred_short_raw", 0) or 0), 6),
            "proba_long_cal":   round(float(getattr(row, "pred_long_cal",  0) or 0), 6),
            "proba_short_cal":  round(float(getattr(row, "pred_short_cal", 0) or 0), 6),
            "proba_long_cal_dec":  round(float(getattr(dec, "p_buy_cal",  0) or 0), 6),
            "proba_short_cal_dec": round(float(getattr(dec, "p_sell_cal", 0) or 0), 6),
            "state":       str(getattr(row, "state", getattr(row, "market_condition", ""))),
            "macro_regime": str(getattr(row, "macro_regime", getattr(row, "regime", ""))),
            "chop_score":   round(float(getattr(row, "chop_score",      0) or 0), 4),
            "exhaustion_score": round(float(getattr(row, "exhaustion_score", 0) or 0), 4),
            "atr":          round(float(getattr(row, "atr", 0) or 0), 4),
            "action":       str(getattr(dec, "action", "")),
            "score_long":   round(float(getattr(dec, "score_buy",  0) or 0), 6),
            "score_short":  round(float(getattr(dec, "score_sell", 0) or 0), 6),
            "regime3":      str(getattr(dec, "regime", "")),
        }
    ...
    def backtest(self, df_rates: pd.DataFrame, initial_equity: float = 10_000.0, df_is_predicted: bool = False) -> Dict[
        ...
        for i in range(len(df_pred)):
            ...
            # FIX BUG-B: en modo compound, el cap diario se calcula sobre el balance
            # actual. Con compound=False se mantiene initial_equity (correcto).
            _daily_cap_base = float(balance) if self.compound else float(initial_equity)
            if self.max_daily_loss_pct is not None and self.max_daily_loss_pct > 0:
                if daily_realized_pnl <= -self.max_daily_loss_pct * _daily_cap_base:
                    trading_halted_today = True

            if (self.max_daily_profit_pct is not None) and (self.max_daily_profit_pct > 0):
                if daily_realized_pnl >= self.max_daily_profit_pct * _daily_cap_base:
                    trading_halted_today = True
            ...
            if (not trading_halted_today) and (len(positions) < max_positions):
                ...
                # FIX BUG-A: n_steps y action_counts se mueven FUERA del bloque
                # 'if order is not None' del margin check. Antes estaban a indent 20
                # (dentro del margin block), así que solo se contabilizaban decisiones
                # cuya orden pasaba el check de margen — descartando silenciosamente
                # decisiones bloqueadas por MARGIN_INSUFFICIENT o MAX_POSITIONS_SPEC.
                n_steps += 1
                if dec is None:
                    final_action = 'none'
                else:
