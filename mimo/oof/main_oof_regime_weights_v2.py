...
def parse_args() -> argparse.Namespace:
    ...
    ap.add_argument("--variant-long", choices=["baseline", "moderate", "moderate_h15", "strong", "auto_soft"], default="moderate")
    ap.add_argument("--variant-short", choices=["baseline", "moderate", "strong", "auto_soft"], default="moderate")
...
def _compute_soft_regime_weights_local(
    ...
    *,
    ...
    if len(states_s) != len(y):
        raise ValueError(
            f"_compute_soft_regime_weights_local: len(states)={len(states_s)} != len(labels)={len(y)}"
        )
    ...
    regime_weights_sample = states_s.map(learned_map).fillna(1.0).to_numpy(dtype=np.float32)
...
def compute_soft_regime_weights(
    ...
    *,
    ...
    helper_fn = getattr(Helper, "compute_soft_regime_weights", None)
    ...
    return _compute_soft_regime_weights_local(
        states=states,
        labels=labels,
        min_weight=min_weight,
        max_weight=max_weight,
...
def main() -> None:
    ...
    holdout_from = datetime(2025, 2, 1)
    holdout_to = datetime(2025, 3, 10)
