"""
resume_deploy_full.py
─────────────────────────────────────────────────────────────────────────────
Script de RECUPERACIÓN — arranca directamente en el paso deploy_full.

Asume que ya existen en disco (train_only) los artefactos de LONG y SHORT:
  - model_<release>_long.keras / model_<release>_short.keras
  - oof_calibrator_<release>_long.joblib / ..._short.joblib
  - percentiles_<release>_long.json / ..._short.json
  - oof_<release>_long.parquet / ..._short.parquet
  - oof_meta_<release>_long.json / ..._short.json
  - scalers_<release>/ (directorio con seq_short.json, seq_long.json, context.json)
  - _optuna_cache/<study_name>/<side>/trial_XXXXX/ (caché OOF del mejor trial)

Lo que hace este script:
  1. Reconstruye trainer con reload=True para cargar best_params desde Optuna DB
  2. Reconstruye TrainerArtifacts para long y short desde los ficheros en disco
  3. Verifica el caché OOF del mejor trial (necesario para reuse_best_trial_oof=True)
  4. Salta directamente a deploy_full con reuse_best_trial_oof=True
     → EVITA regenerar el OOF sobre el dataset completo (causaba OOM en VRAM)
     → Reutiliza el calibrador del mejor trial de Optuna, que ya es válido
  5. Ejecuta validación final

CAMBIOS respecto a versión anterior:
  - reuse_best_trial_oof=True en ambos prepare_production_model (fix OOM)
  - Verificación explícita del caché OOF antes de lanzar
  - Limpieza de memoria más agresiva entre LONG y SHORT
  - free_memory() antes y después de cada paso crítico
─────────────────────────────────────────────────────────────────────────────
"""
...
def recalibrate_deploy_side(
    ...
    y_true = sequences[side]['labels'].astype(int)
    ...
    # Sobrescribir / regenerar percentiles json en deploy_dir
    ...
    # Guardar dataset de calibración final para análisis posterior
    data_out = deploy_dir / "data" / f"deploy_calibration_tail_{release}_{side}.parquet"
    data_out.parent.mkdir(parents=True, exist_ok=True)

    df_tail = df_calib_prep.iloc[-len(y_true):].copy()
...
def validate_after_training_with_metrics(artifacts_long, artifacts_short, df_full, out_dir,
    ...
    try:
        ...
        with open(artifacts_long.calibrator_path,  'rb') as f: cal_long  = joblib.load(f)
        with open(artifacts_short.calibrator_path, 'rb') as f: cal_short = joblib.load(f)
        ...
        for side in ['long', 'short']:
            ...
            try:
                ...
                scalers_used = _load_pipeline_scalers_for_side(pipeline_side, artifacts_long.model_path if side == 'long' else artifacts_short.model_path, side)
                print(f"     · scalers: {scalers_used}")
                ...
                X_val  = [sequences[side]['seq_short'], sequences[side]['seq_long'],
                          sequences[side]['context'],   sequences[side]['time']]
                ...
                # ── Threshold OOF (transferido desde el mejor trial) ──────────
                # El threshold correcto para evaluar el modelo de producción es el
                # que se calculó durante el OOF, no uno re-optimizado sobre este
                # val set (lo que sería look-ahead y daría métricas infladas/degeneradas).
                # Lo leemos desde percentiles_path del artefacto correspondiente.
                artifacts_side = artifacts_long if side == 'long' else artifacts_short
                oof_threshold = None
                try:
                    import json as _json
                    with open(artifacts_side.percentiles_path) as _f:
                        _pct = _json.load(_f)
                    # El threshold OOF se guarda en _meta o en el campo threshold del artefacto
                    oof_threshold = _pct.get('_meta', {}).get('selected_threshold') or                                     _pct.get('selected_threshold')
                except Exception:
                    pass

                # Threshold óptimo sobre val (solo como referencia, NO para métricas reales)
                ...
                # Usar threshold OOF si disponible, si no caer al óptimo sobre val con aviso
                if oof_threshold is not None:
                    best_thr = float(oof_threshold)
                    thr_source = f"OOF transferido ({best_thr:.4f})"
                else:
                    best_thr = val_opt_thr
                    thr_source = f"óptimo val (OOF no disponible) ({best_thr:.4f})"
                    print(f"  ⚠️  [{side.upper()}] threshold OOF no encontrado en percentiles_path, "
                ...
                print(f"     P: {m['precision']:.4f} | R: {m['recall']:.4f} | F1: {m['f1']:.4f} | signal_rate: {m['signal_rate']:.4f}")
...
# ─────────────────────────────────────────────────────────────────────────────
# Helpers para reconstruir TrainerArtifacts desde disco
# ─────────────────────────────────────────────────────────────────────────────
...
def load_artifacts_from_disk(train_dir: Path, release: str, side: str) -> TrainerArtifacts:
    """
    Reconstruye un TrainerArtifacts leyendo los ficheros ya guardados en train_only.
    ...
    if os.path.exists(oof_df_path):
        try:
            ...
            mask = isfinite(df_oof['oof_proba_cal']) & isfinite(df_oof['signal'])
            if mask.sum() > 0:
...
def verify_optuna_cache(train_dir: Path, study_prefix: str, release: str, side: str) -> bool:
    ...
    Necesario para que reuse_best_trial_oof=True funcione sin regenerar el OOF.

    Devuelve True si el caché está completo, False si hay que regenerar.
    """
    ...
    # Buscar cualquier trial con los 3 ficheros necesarios
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Recupera deploy_full evitando OOM al permitir ejecutar long/short por separado."
    )
    parser.add_argument(
        '--side',
...
def train_single_side(trainer, df_rates, side):
    print(f'\n[DEPLOY] Preparing production {side.upper()} model based on FULL period')
...
if __name__ == "__main__":
    ...
    print(f"\n🚀 Modo de ejecución: side={args.side} | skip_validation={args.skip_validation} | validate_only={args.validate_only}")

    # ── Configuración — debe ser idéntica a main_oof.py ──────────────────────
    general = Config(
        release='200377',
        use_oof=True,
        oof_splits=5,  # 5
    ...
    # ── Reconstruir trainer con best_params desde Optuna DB ──────────────────
    ...
    trainer = OptunaOOFTrainer(
        ...
        seed=42,
        reload=True,   # carga best_params desde Optuna DB sin re-entrenar
    ...
    if args.validate_only:
        ...
        # ── Reconstruir TrainerArtifacts desde disco ──────────────────────────────
        ...
        # ── Verificar caché OOF del mejor trial ───────────────────────────────────
        # Necesario para reuse_best_trial_oof=True. Si no existe el caché, el trainer
        # intentará regenerar el OOF sobre df_rates completo, lo que causará OOM en VRAM.
        ...
        if holdout_report_path.exists():
            ...
            #holdout_report_long  = trainer.evaluate_holdout(artifacts_long_train,  df_hold, side='long')
            holdout_report_long = {
            ...
            free_memory()
            #holdout_report_short = trainer.evaluate_holdout(artifacts_short_train, df_hold, side='short')
        ...
        # ── Limpiar memoria antes del deploy_full ─────────────────────────────────
        ...
        Helper.save_meta(
            {
                ...
                'note':       'trained after holdout approval; holdout no longer independent for this artifact. '
                              'reuse_best_trial_oof=True: calibrator from best Optuna trial reused to avoid VRAM OOM.',
            },
        ...
        if args.side in ('long', 'both'):
            artifacts_long_deploy = train_single_side(trainer, df_rates, 'long')
            release_artifact_refs((locals(), ['artifacts_long_train']))

        if args.side in ('short', 'both'):
            artifacts_short_deploy = train_single_side(trainer, df_rates, 'short')
